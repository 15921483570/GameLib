// 19_clip_tilemap.cpp - Clip Rectangle with Tilemap Windows
//
// Based on 14_tilemap.cpp, but draws everything through several clip windows:
//   - Main scene window: two-layer scrolling tilemap + trees + character sprite
//   - Shape window: lines / circles / triangles clipped by SetClip
//   - Text window: DrawTextFont / DrawText / DrawPrintf clipped by SetClip
//
// Controls: <- -> or A/D move character, ESC to quit
...
int main()
{
    ...
    int charSpr = game.LoadSprite("../assets/character.png");
    int treeSpr = game.LoadSprite("../assets/tree.png");
    ...
    while (!game.IsClosed()) {
        ...
        game.DrawText(24, 6, "19_clip_tilemap.cpp - SetClip windows for scene, shapes, and text", COLOR_WHITE);
        ...
        {
            int clipX = game.GetClipX();
            int clipY = game.GetClipY();
            int clipW = game.GetClipW();
            int clipH = game.GetClipH();
            ...
            game.DrawPrintf(TEXT_X + 8, TEXT_Y + 110, COLOR_LIGHT_GRAY,
                            "Move with A/D or arrows. ESC quits.");
        }
        game.ClearClip();
